#!/usr/bin/env python3
"""
Compile the paper from markdown to PDF with embedded figures.
...
INPUT_LATEX = 'oracle_axiom_equivalence.latex.md'
FIGURES = []
...
def compile_to_pdf(input_file=INPUT_LATEX, output_file='oracle_axiom_equivalence.pdf'):
...
def compile_single_column(input_file=INPUT_LATEX, output_file='oracle-axiom-equivalence-single.pdf'):
...
def compile_to_html(input_file=INPUT_LATEX, output_file='oracle_axiom_equivalence.html'):
...
def compile_for_arxiv(input_file=INPUT_LATEX, output_dir='arxiv_submission'):
...
def main():
    ...
    if not Path(INPUT_LATEX).exists():
        print("\n✗ Input file INPUT_LATEX not found!")
        sys.exit(1)
    ...
    print("=" * 70)
    if success_single:
        print("✓ Single-column PDF")
    if success_two_col:
        print("✓ Two-column PDF")
    if success_arxiv:
